...
import com.easarrive.quartz.aws.util.Constant.SQS.S3.URL.Access.Goods;
 ...
 *
 */
public class ThumborImageGoodsService extends AbstractThumborImageService<String, String> {

    public ThumborImageGoodsService() {
        super(Goods.SOURCE_FORMAT, Goods.TARGET_FORMAT);
    }

    @Override
    ThumborRequest getThumborRequest(String redisKey, String redisValue) throws ThumborException {
        String key = redisValue.substring(1);
        String accessUrl = String.format(Goods.SOURCE_FORMAT, this.getBucketName(), key);
